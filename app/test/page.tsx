...
import { Play as PlayIcon, CheckCircle as CheckCircleIcon, XCircle as XCircleIcon, Clock as ClockIcon } from 'lucide-react';
...
export default function TestPage() {
  ...
  const updateTestResult = (api: string, result: Partial<TestResult>) => {
    setTestResults(prev => {
      ...
      if (index >= 0) {
        prev[index] = { ...prev[index], ...result } as TestResult;
        return [...prev];
      } else {
        return [...prev, { api, status: 'pending', message: '', ...result } as TestResult];
      }
    });
  };
